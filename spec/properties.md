...
| | |
|---|---|
| **Claim** | Every operation that takes effect on a governed system is the authorized execution content of a valid, unexpired, unredeemed Execution Change Contract produced by `C7`. |
...
| **Invariant** | I1 (Reachability), realized at the boundary by the execution surface `ES(t)` of Part II §6.2: `ES(t) = { op \| ∃ ECC r in C7's active registry at t such that op ∈ exec(r) }`, and `C6` MUST block any operation not in `ES(t)`. |
...
| **Counterexample / test** | NT-001 (no ECC → blocked), NT-002 (expired ECC → blocked), NT-003 (replay → blocked), and the Part IV network-layer P4 verification. **A falsification is any demonstrated path by which an operation reaches a governed system in the boundary without a redeemed ECC.** |
| **Evidence produced** | `EXECUTION_BLOCKED` events with a typed `block_reason`; the absence of any `EXECUTION_AUTHORIZED` without a corresponding redeemed ECC; governed systems' own access logs, cross-checked per §29.4. |
...
principle. What CROA adds is the *artifact* that carries the decision across the boundary (the ECC)
...
| | |
|---|---|
| **Claim** | At most one execution is authorized per Execution Change Contract, and at most one (or `N` under a declared `bounded-count` policy) per authorization artifact — under concurrency, across every enforcement instance, in every deployment topology. |
...
| **Invariant / normative basis** | Part II §4.8 (no dedicated `In` identifier): redemption MUST be a single linearizable compare-and-swap claiming `ecc.id` (and, for a governed exception, `ecc.auth_ref`) in one indivisible operation. Query-then-act is explicitly prohibited as a time-of-check-to-time-of-use race. |
...
| **Counterexample / test** | NT-007 step 4: present the same `ecc.id` (and the same `auth_id`) simultaneously to two `C6` instances; **at most one** `EXECUTION_AUTHORIZED`. **A falsification is any topology or timing under which two executions are authorized from one commitment or one authorization.** |
| **Evidence produced** | Exactly one `EXECUTION_AUTHORIZED` per `ecc.id`; `EXECUTION_BLOCKED` with `ECC_ALREADY_REDEEMED` / `AUTHORIZATION_ALREADY_REDEEMED` for every loser. |
| **Does NOT establish** | **General consistency of cumulative (`TP-C`/`TP-X`) trajectory state.** Two governed actions evaluated concurrently carry *distinct* `ecc.id`s, so the redemption CAS does not relate them. In **v1.0 as published** nothing else relates them either, so each may be evaluated against the same pre-increment aggregate and both admitted, jointly crossing a threshold neither crossed alone; that is errata E-11. The next version adds a minimum requirement that the read-evaluate-increment cycle be serialized per **accumulation key**, which closes that specific case. Neither version claims general serializability between a decision and the state at which its effect commits. See RQ-16. Nor does it establish that a ECC remains *appropriate* at redemption: `C6` re-validates signature, expiry, redemption status, revocation, and `ecc.invariant_set_version` against the invariant **registry**, but not the runtime trajectory state or the policy-artifact version (open residual; RQ-17). |
...
| **Preconditions** | Read access to `C5` (which §29.4 distinguishes sharply from the implementing party's *cooperation*); the `C5` signing key held outside the governed agent's trust domain (Appendix R Inv. 4); external anchoring of the sealed-segment head (TH-4). Additionally, for a governance decision that depends on external factual state, independent reconstruction requires the exact evaluated state to be verifiably bound to the `C5` decision record (e.g. via `event.invariant_state`). That binding may be achieved either by recording the state directly or by recording an immutable/content-addressed reference sufficient to retrieve and verify the exact state used. |
...
| **Evidence produced** | The `C5` extract, its chain verification result, and the ECC-integrity sample. |
| **What the harness shows, and does not** | Since September 2026 the Minimal Reference Harness implements the Appendix G.2.4 correlation and rejects logs with a double authorization, an orphan execution, a deleted event or a tampered one ([`known-defects-harness.md`](known-defects-harness.md) H-04). That demonstrates the *shape* of reconstructability in a mock. It does **not** evidence the property for a real deployment, and it does not touch capture completeness — the row below stands unchanged. Before that fix, `verify()` recomputed the chain and nothing else, and an audit produced a valid-verifying log with two executions from one single-use authorization. |
...
| **Commit-time authorization freshness** — that a ECC still reflects current trajectory state and policy-artifact version at redemption | Not established. `C6` re-checks the invariant **registry** version, not runtime state | RQ-17; Part II §4.8 |
