...
- **Govern the record's changes.** Updates to the golden record are change events under Policy Update (§7.2) (Part III Chapter 16); the record carries a version, and `gga.resolved_entities` records the version/state at resolution time so decisions are reconstructable (I3).
