...
**Execution Change Contract (ECC)** — the immutable, content-addressed, signed artifact produced by C7 that encodes exactly one authorized action and its constraints. The **only** thing allowed to cross the execution boundary. (Named "RFC / Request for Change" in pre-1.0 drafts; unrelated to IETF RFCs.)
...
**Execution surface `ES(t)`** — the set of operations for which a valid, unredeemed Execution Change Contract exists at time *t*. `C6` blocks anything outside it, and it never expands without a permit decision (Part II §6.2).
...
**Federated Context Registry** — the registry of legitimate endpoints, resources, and entities against which C3 grounds a request. Its completeness is an open practical question (RQ-7).
