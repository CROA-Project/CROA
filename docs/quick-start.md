...
The vendor-neutral **Minimal Reference Harness** demonstrates a *reduced* control plane — mock `C1` to `C7`, including `C4` (trajectory state) and an admission predicate (RBAC per §4.9.1), but with **no authentication** behind that predicate, and a `C6` gateway that holds the only channel to a governed system running in its own process — obtained by process isolation, not by network policy — using no commercial software. Since v1.0.1 it ships **all eight** reference negative tests, NT-001 to NT-008. It still does not demonstrate C1–C7 as a deployment would; see [`spec/known-defects-harness.md`](../spec/known-defects-harness.md) for what it does and does not establish, in particular H-05 and H-06.

> **What the harness is, precisely.** It is a *demonstrator*: a self-contained mock of the control plane, so its assertions check that the mock behaves as the specification describes. That makes it a useful way to see the mechanism and the event log — it is **not** evidence that a real implementation has the properties, and it is not an experiment. NT-005 (ambiguous `E3` verdict), NT-006 (trajectory) and NT-008 (authority non-expansion) are now implemented, with the semantic analyzer, `C4` state and delegation model each present in mock form. NT-007 covers replay, double pre-compilation, concurrent redemption and scope widening. Since 7 September 2026, NT-001 and NT-003 are also asserted against a governed system in another process: the direct path fails inside `connect(2)` and the assertions are on that system's own log, not on a return value. What that still does **not** mean: the path is unreachable because no address exists, not because a network policy forbids it, so property **P4** is demonstrated in the harness and remains unproven for a deployment (H-06). The evidence buckets CROA has and has not filled are listed in [`evidence/README.md`](../evidence/README.md).
