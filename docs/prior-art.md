...
|---|---|---|---|
| **Policy engines** — OPA/Rego, AWS Cedar | Deterministic, declarative allow/deny for a single request | A bare decision is advisory unless something structurally stops the agent from acting on a deny. CROA compiles a permit into a single-use, content-addressed, signed **Execution Change Contract** (C7) that the boundary (C6) alone will admit, and adds context grounding (C3), cumulative trajectory evaluation (C4), and a conformance-bearing audit chain (C5). | **Implements C2.** CROA = a policy engine + C3/C4/C7/C6/C5, composed. |
| **Capability-based security** (object capabilities, capability tokens) | Unforgeable, least-authority handles; possession = authority | Capabilities bound *static* authority; CROA's Execution Change Contract is a *per-action, single-use, context-grounded, audited* capability whose scope is verified against the current invariant set at redemption, and whose use is recorded. It also adds cross-action **trajectory** limits a capability can't express. | **Kindred idea, extended.** The ECC is a governed, evidence-bearing capability, not a standing one. |
...
### 2.0 CROA's own dated record, and what precedes it

A related-work section without dates silently concedes priority. CROA's public record is:

| Record | Date | Contains |
|---|---|---|
| **CROA Framework: Deterministic AI Governance and Mitigation of Technical Sycophancy** — Durand & Smith, working paper, DOI [`10.5281/zenodo.19898196`](https://doi.org/10.5281/zenodo.19898196) (v2; V1 is [`10.5281/zenodo.19846872`](https://doi.org/10.5281/zenodo.19846872)), CC BY 4.0, OpenAIRE-indexed | **submitted 27 April 2026, published 28 April 2026** | **Technical Sycophancy** and **Agentic Sprawl** as named failure modes; agentic execution modelled as a **constrained state-transition system**; actions conditioned on pre-validated invariants; validated transitions formalised into contracts before execution; and the central thesis verbatim — *"unsafe transitions are not merely rejected at runtime; they are excluded from the executable state space by construction."* Describes the earlier four-layer architecture under the name *Cognitive RFC Orchestration Architecture*. |
...
**What priority does not do.** It does not make a claim novel: several of the works that *precede*
CROA occupy ground CROA also occupies, and no filing date changes that. It does not extend to the
classical foundations of §2.1. And it says nothing about evidence — a dated record of a thesis is not
...
| **One-way attenuating credentials** | Macaroons (NDSS 2014); Biscuit; SPKI/SDSI chain discovery (JCS 9(4), 2001); Delegation Logic (ACM TISSEC 6(1), 2003); RFC 8693 OAuth 2.0 Token Exchange. | A Execution Change Contract is, structurally, a per-action attenuated capability carrying an audit obligation. CROA already cites RFC 8693 (Appendix O §O.5). |
...
| **Certificate-gated execution** *(May 2026 — follows)* — Liu, Wang & Capponi, *No Certificate, No Execution*, arXiv:2605.24462 | A proposal–certification–execution architecture in which execution is conditional on a certificate, with the explicit observation that generation is not permission and that individually permissible actions may compose into an impermissible trace. | **Structurally the closest published analogue of CROA's `C7`/`C6` and the Execution Change Contract**, and a reviewer will find it immediately. Convergent on: an artifact that must exist for execution to proceed, and non-compositionality of permissibility. Divergent: CROA additionally registers a trajectory profile per invariant, carries a conformance model, and mandates atomic redemption across enforcement instances. Neither derives from the other. |
| **Composing stateful pre-action controls** *(Aug 2026 — follows)* — *One Gate Is Not Enough*, arXiv:2608.18360 | Argues a single pre-action gate is insufficient and studies the composition of stateful controls. | Directly on the composition question that CROA's I8 and P-B address. |
| **Proof-carrying agent actions** *(Jun 2026 — follows)* — arXiv:2606.04104 | Model-agnostic runtime governance in which an action carries its own justification. | Adjacent to the Execution Change Contract as a carrier of authorization and evidence. |
...
**Differentiating, so far as this survey reaches.** The five claims below are the framework's remaining case for distinction. Each is a negative statement over an incomplete survey, so each is stated as *we are not aware of*, not as *no work does*. If you know of a counterexample to any of them, that is the single most useful correction this page can receive — open an issue.
...
1. **Evaluability classes as a registered property of every invariant.** `E1` syntactic / `E2`
   ...
   and the measured ambiguity rate treated as a governed quantity. We are not aware of other work that
   makes decidability a declared attribute of each individual invariant.
...
3. **Claim-scope discipline as a governance obligation.** The conditioned T1 statement, the §28.6
   ...
   the unconditioned claim a *governance* violation. We are not aware of another framework that
   normatively binds its own claim wording.
