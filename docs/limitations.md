...
|---|---|
| **Author-controlled testing** | Present. The Minimal Reference Harness (the published cooperative and adversarial reference tests) and the founding comparative study. *CROA-Bench — an internal conformance-metric seed with rule-based mock adapters — is **not yet published**, so a reader cannot inspect it and it should not be counted as evidence until it is.* |
...
| **Adversarial testing** | **Present, author-run, and it began by finding defects rather than confirming properties.** An independent audit reported three defects in the reference harness; the project reproduced them ([`evidence/harness-defects/`](../evidence/harness-defects/)), found a fourth, fixed all four, and shipped an adversarial suite: forged signatures, forged content addresses, mutated operations, subject substitution, deleted and tampered events, orphan executions, double authorization, and two 100-thread races. **What is still missing is the part that matters most** — every one of those tests was written by the project against its own artifact, from someone else's list. No outside attempt to break it exists. While the current harness now exercises the **P4** mechanism with an isolated governed-system process where the C6 gateway is the only addressable path and assertions are made on external effects in the governed system log, this isolation mechanism relies on process topology, not a deployment network policy. Therefore, this demonstrates the mechanism but does not establish P4 for any real deployment (see [`spec/known-defects-harness.md`](../spec/known-defects-harness.md) H-06). |
